...
The height of a tree node is determined by the \emph{time}; at which the corresponding ancestor that corresponds to the node was born. So,
...
200 generations. See \cite{Wakeley2009} for details on the biological interpretation of effective population size.
...
therefore very often interesting not just in the genealogies that are
...
migration patterns. This population structure is declared in \msprime\ using the
...
not stored by default, as they may consume a substantial amount.
...
per unit of sequence length, per generation. It is usually useful to
...
correspondence should not be taken too literally. Note also that
the \texttt{length} parameter is not connected to the number
of mutational \emph{sites}; any number of mutations can occur on a
given sequence length, depending on the mutation rate that is specified.)
For this example, we
