...
\begin{pythoncode}
ts = msprime.sim_ancestry(10, population_size=1e4,
                          sequence_length=1e5,
...
demography.add_mass_migration(time=50000, source=1,
...
sns.histplot(ibd_segments(ts, 0, 1), label="Within population",
...
row = 0
for rho in rhos:
    replicates = msprime.sim_ancestry(1,
...
def run_sims(m, num_loci=1, theta=0):
    demography = msprime.Demography.island_model([1, 1],
                                                migration_rate=m)
...
def approx_jSFS(m):
  ...
  for rep_index, ts in enumerate(reps):
      B[rep_index] = ts.allele_frequency_spectrum([[0,1],[2,3]],
                                          mode='branch',
                                          polarised=True,
...
with cf.ProcessPoolExecutor(max_workers=4) as executor:
  futures = [executor.submit(approx_jSFS, prior)
             for prior in prior_m]
  prior_jSFS = np.array([f.result()
                         for f in cf.as_completed(futures)])
