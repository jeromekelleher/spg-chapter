...
Some models, like \texttt{SweepGenicSelection} are only defined for a specific time interval and do not run until coalescence. The following example shows
...
\texttt{duration} parameters being specified.
...
and then switch to the \emph{StandardCoalescent} model for the remaining generations. Figure \ref{fig:politomy} shows the tree
...
\begin{figure}
\begin{center}
\includegraphics[width=0.48\textwidth]{images/politomy.pdf}
\end{center}
\caption{\label{fig:politomy} Tree simulated with a combination of the \emph{DTWF} model and a \emph{StandardCoalescent} model. Politomy is observed.}
\end{figure}

\runinhead{Selective sweeps}\

We have seen  in section~\ref{model-duration-and-model-completion} how to set up and use a selective sweep model. However, selective sweeps are
usually simulated using a combination of the selective sweep model that runs for a specific period and a neutral model (usually the standard
coalescent model) that runs until coalescence. In this example, we will do a simple selective sweep simulation.
