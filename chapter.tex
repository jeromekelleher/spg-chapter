...
with cf.ProcessPoolExecutor(max_workers=4) as executor:
     futures = [executor.submit(approx_jSFS, m) for m in prior_m]
...
The Demes format~\citep{gower_demes_2022} is intended to become a standard for writing
...
The previous code block loads the example demes model of figure~\ref{fig-demes-plot} from a YAML file.
The converted \texttt{Demography} is then investigated using the \texttt{debug()} method.

\begin{footnotesize}
\begin{verbatim}

Epoch[0]: [0, 500) generations
Populations (total=3 active=2)
start	end	growth_rate	A	B
A	5000.0	5000.0	0	0	0
B	10000.0	400.0	0.00644	0.0001	0
Events @ generation 500
time	type	parameters	effect
500	Population parameter change	population=B, initial_size=2000,
    growth_rate=0	initial_size → 2e+03 and growth_rate → 0 for
    population B

Epoch[1]: [500, 1e+03) generations
Populations (total=3 active=2)
start	end	growth_rate	A	B
A	5000.0	5000.0	0	0	0
B	2000.0	2000.0	0	0.0001	0
Events @ generation 1e+03
time	type	parameters	effect
1000	Population parameter change	population=X, initial_size=5000
        initial_size → 5e+03 for population X
1000	Population Split	derived=[A, B], ancestral=X	Moves all
        lineages from derived populations 'A' and 'B' to the
        ancestral 'X' population. Also set the derived populations
        to inactive, and all migration rates to and from the derived
        populations to zero.
1000	Migration rate change	source=B, dest=A, rate=0
        Backwards-time migration rate from B to A → 0

Epoch[2]: [1e+03, inf) generations
...
The demography debugger output shows three epochs. Epoch[0] corresponds to the
time interval [0, 500) generations ago, in which deme B has an initial size of
10000 at time 0 and backwards in time, shrinks in size to 400 individuals at
time 500. Lineages sampled from deme B migrate into deme A at a rate of 0.0001
per generation. In Epoch[1], [500, 1000) generations ago, both A and B have
constant sizes, and migration is the same as in Epoch[0]. At the end of Epoch[1],
at generation 1000, lineages in demes A and B are moved into deme X and migrations
are turned off. Finally, Epoch[2] shows that only deme X remains.

Demes models have time in units of “time ago”, and when using generations as the time units,
the times will directly correspond to msprime’s notion of time: time 0 is “now” and time
values increase towards the past. However, in contrast to msprime, Demes uses forwards-time
conventions. Noteworthy differences between Demes and msprime include:
\begin{itemize}

    \item Each deme has its own list of epochs, whereas in msprime’s \texttt{DemographyDebugger}, epochs are
    time intervals that apply across the entire demography.

    \item Demes uses \texttt{start\_time} and \texttt{end\_time} to indicate time intervals, but
     the \texttt{start\_time} has a value which is larger (more ancient) than the \texttt{end\_time}.

     A Demography object can also be exported to a demes.Graph, using the Demography.to\_demes()
     method.
...
approach to assigning samples, and allows samples to be assigned to
...
sns.histplot(ibd_segments(ts, 0, 1), label="Within population", kde=True,
    ...
    alpha=.4, edgecolor=(1, 1, 1, .4), bins=70)
...
sns.histplot(simulation, kde=True,
    stat="density", kde_kws=dict(cut=3),
    alpha=.4, edgecolor=(1, 1, 1, .4), bins=25)
...
\begin{tabular}{ll}
Jupyter notebook & \url{https://github.com/StatisticalPopulationGenomics/msprime} \\
Documentation & \url{https://msprime.readthedocs.io/en/stable/} \\
GitHub & \url{https://github.com/tskit-dev/msprime} \\
Mailing list & \url{https://groups.google.com/forum/\#!forum/msprime-users} \\
\end{tabular}
