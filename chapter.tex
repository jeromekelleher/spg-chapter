...
\hline
\end{tabular}
\caption{Ancestry models supported by \msprime.}
\label{tab:ancestry-models}
\end{table}

\subsubsection{Single models}\label{single-models}

It is more common to run simulations using a single ancetory model. The model used for the simulation can be specified by passing an argument
from table~\ref{tab:ancestry-models} as the \texttt{model} parameter to the \texttt{sim\_ancestry} function.The following example shows how
to simulate the coalescent process using the \emph{SmcApproxCoalescent} model.
...
Here, a simulation run for ten generations, startin from generation 5 until generation 15. As shown in Figure~\ref{fig:start-duration}, simulations stoped
...
Some models, like \texttt{SweepGenicSelection} are only defined for a specific time interval and do not run until coalescence. The following example showes
how to simulate the coalescent process using the \emph{SweepGenicSelection}.
...
\begin{figure}
\begin{center}
\includegraphics[width=0.48\textwidth]{images/sweep.pdf}
\end{center}
\caption{\label{fig:SweepGenicSelection} Ancestory simulation using the \emph{SweepGenicSelection} model.}
\end{figure}

Figure~\ref{fig:SweepGenicSelection} shows the result of the simulation. The simulation stops before coalescence is reached, and without the \texttt{end\_time} or
\texttt{duration} parameters bieng specified.

\subsubsection{Multiple models}\label{multiple-models}

Different ancestory models can be combined using \msprime\; for example, the DTWF model can be used to simulate the more recent
...
A pedigree records the ancestory relationships between individuals, which can limit the possibilities of genetic inheritance. It is possible to
simulate ancestory conditioned on a fixed pedigree using \msprime\ by using the \texttt{FixedPedigree} model. This model limits the transfer of
genetic material from ancestors to descendants, while still allowing for recombination. Recombination events occur randomly; therfore, each of
...
In the following code block, this pedigree is used to simulate ancestory using the \texttt{FixedPedigree} model.
...
The result of the simulation is shown in Figure~\ref{fig:pedigree-simulation}, where the node labels show the individual ID and the node ID;
 because of recombination, the result is two tress for the same fixed pedigree.
