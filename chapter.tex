...
with cf.ProcessPoolExecutor(max_workers=4) as executor:
    futures = {executor.submit(approx_jSFS,m): i
        for i, m in enumerate(prior_m)}
    prior_jSFS = [None] * num_prior_draws
    for f in cf.as_completed(futures):
        prior_jSFS[futures[f]] = f.result()

prior_jSFS = np.array(prior_jSFS)
print(prior_jSFS[0])
\end{pythoncode}
...
ci_m = np.percentile(post_m, 2.5), np.percentile(post_m, 97.5)
sns.histplot(prior_m, label="Prior", stat="density")
sns.histplot(post_m, label="Posterior", stat="density")
\end{pythoncode}
