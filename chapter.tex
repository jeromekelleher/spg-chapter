...
Migration rates are confusing! Are they fractions or rates? If they're fractions, why can we have values
bigger than 1?
...
Because analytical approaches relating sequence variation to mechanistic models
...
First, comparisons between analytical results and simulations serve as an important
...
that we provide to define the simulation that is
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.4\textwidth]{images/simulations_5_0.pdf}
\end{center}
...
using the IPython SVG function on the \texttt{tree.draw()} method, which
by default returns a depiction of the tree rendered in SVG. (The
\texttt{tree.draw()} method also supports rendering trees in plain text.)

The output of one random realisation of this process is shown in
Figure~\ref{fig-simple-tree}, where we can see that
...
which means that all times are scaled by \(4N_e\) (rather than $2N_e$ for a
...
indefinitely. This is particularly important in simulation base
...
therefore very often interested not just in the genealogies that are
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.4\textwidth]{images/simulations_19_0.pdf}
\end{center}
...
When comparing simulations to analytical results, it is very important to be aware of the way in which the
...
factor of two if we are to compare with analytical predictions. For
...
\includegraphics[width=0.3\textwidth]{images/simulations_26_0.pdf}
\label{fig-trees-migration-asymmetric}}
\qquad\qquad
\subfloat[][Three population island model]{
\includegraphics[width=0.3\textwidth]{images/simulations_28_0.pdf}
...
Figure~\ref{fig-trees-island-model} shows tree produced by this code
chunk. Here we sample three nodes from population 0, but because there is a lot
...
We can see that node zero experienced very few migration events before it
...
\includegraphics[width=0.3\textwidth]{images/simulations_34_0.pdf}
\label{fig-trees-migration-rate-change}}
% \qquad
\subfloat[][Mass migration]{
\includegraphics[width=0.3\textwidth]{images/simulations_37_0.pdf}
\label{fig-trees-mass-migration}}
% \qquad
\subfloat[][Admixture]{
\includegraphics[width=0.3\textwidth]{images/simulations_39_0.pdf}
...
% \begin{wrapfigure}[13]{hr}{0.4\textwidth}
%   \begin{center}
...
ts = msprime.simulate(
    samples=[
        msprime.Sample(0, 0), msprime.Sample(0, 0),
        msprime.Sample(0, 0),
...
correlated trees. To process the trees, we simply need to consider
...
occur exactly at the integer boundaries between these loci.
...
reasonable approximation of the real distribution of block lengths.
Because the main cost of this function is in iterating over all the
trees in the sequence, would be more efficient to keep track of the MRCAs
...
completes, we then see that such rare alleles are very common. (We reiterate that \msprime\
generates mutations under the infinitely many sites model so that each mutation occurs at a
unique site. In more complex situations where we have back or recurrent mutations
this simple approach will not work.)

A very powerful feature of this sample-counting approach is that we can
...
equal to the within-population count, we know that this mutation is private
to the population.
...
In this section we show some examples of validating simple analytical
...
demonstration of how to proceed when comparing analytical results to
...
All analytical predictions are taken from \cite{wakely2008coalescent}.
...
\begin{center}
    ...
    \caption{Comparisons of the distribution of simulated total branch lengths
        with analytical results. (A) The full distribution of simulated
...
an analytical expression. In the following code chunk we define the analytical
prediction for the total branch length distribution, and
plot the simulated distribution for a sample of size 20 along with
this prediction. The results are shown in
Figure~\ref{fig:segsites-norecomb}B. We can see an excellent agreement between
the smoothed kernel
density estimate produced by Seaborn and our analytically computed value.
...
We cannot directly observe branch lengths, and so we are usually more
...
results are not known in general for many quantities under recombination,
...
def approx_jSFS(m):
    ...
    for rep_index, ts in enumerate(reps):
        ...
        for tree in ts.trees(tracked_samples=samp1):
            # Note that this will be inefficient if we have
            # lots of trees. Should use an incremental update
            # strategy using edge_diffs in this case.
            for u in tree.nodes():
                n1 = tree.num_tracked_samples(u)
                n2 = tree.num_samples(u) - n1
                if tree.parent(u) != msprime.NULL_NODE:
                    B[rep_index, n1, n2] += tree.branch_length(u)
...
\begin{center}
    \parbox{5cm}{
    \includegraphics[width=5cm]{images/inference-example_13_0.pdf}
    \begin{center}(A)\end{center}
    }%
    \qquad
    \parbox{5cm}{
    \includegraphics[width=5cm]{images/inference-example_25_0.pdf}
    \begin{center}(B)\end{center}
    }
...
and to thank Yan Wong and Joseph Marcus for detailed and insightful feedback.
