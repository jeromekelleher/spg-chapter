...
each locus, i.e. the blockwise site frequency spectrum bSFS (sensu Lohse
et al 2016). Note that higher level population genetic summaries, e.g.~pairwise
measures of divergence and diversity such as \(D_{XY}\) (Nei 1987) and
\(F_{ST}\) (Wright 1950) or multi-population \(F\) statistics (Durand et
al 2011, Patterson 2012) which are often used in ABC inference are just
further (and lossy) summaries of the jSFS.
...
\end{pythoncode}
    ...
    One could base inference on the bSFS (Lohse 2016 et al, Beeravolou et al
2017), but we will for the sake of simplicity use a simpler (and lossy)
...
\end{pythoncode}
    ...
    To illustrate a simple ABC inference scheme, we will focus on a single
...
mean length of genealogical branches (as in the bottleneck section
above) across 100 simulation replicates (\texttt{num\_reps}). Below we
draw 10,000 \(m\) values from the prior and use the function
\texttt{run\_sims} and \texttt{approx\_jSFS} to approximate the jSFS for
each. We assume an exponential distribution, a common choice of prior
(Hey \& Wakeley 2002).
...
Here we run 100 simulation replicates for each of the 10,000 prior
$m$ values, giving a total of 1 million individual simulations.
...
posterior distribution of \(m\) from the 5\% of simulation replicates
