...
mutated_ts = msprime.sim_mutations(ts, rate=1e-8, random_seed=7, discrete_genome=False)
...
sns.histplot(ibd_segments(mutated_ts, 0, 1), label="Within population", kde=True,
    stat="density", kde_kws=dict(cut=3),
    alpha=.4, edgecolor=(1, 1, 1, .4), bins=70)
sns.histplot(ibd_segments(mutated_ts, 0, 10**5), label="Between populations", kde=True,
    stat="density", kde_kws=dict(cut=3),
    alpha=.4, edgecolor=(1, 1, 1, .4), bins=70)
...
for rho in rhos:
    ...
    for ts in replicates:
        mutated_ts = msprime.sim_mutations(ts, rate=theta/2, discrete_genome=False)

        rho_col[row] = rho
        s_col[row] = mutated_ts.num_sites
        row += 1
...
def run_sims(m, num_loci=1, theta=0):
    ...
    replicates = msprime.sim_ancestry(samples=[
                                msprime.SampleSet(nsamp, population=0),
                                msprime.SampleSet(nsamp, population=1)],
                                demography=demography,
