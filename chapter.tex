...
By default, \msprime\ simulates the coalescent process based on the Hudson model~\citep{hudson1983properties}, refered to as the
...
supported models please refere to the \msprime\ documentation).

The model used for the simulation is specified using the \texttt{model} parameter
in the \texttt{sim\_ancestry} function. Moreover, \msprime\ is also to combine different models; for example, the DTWF model can be used to simulate the more recet
past, while the faster standard model can be used to simulate the more distant past, which provides a decent approximation of the discrete process.
